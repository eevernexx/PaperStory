...
export default function F1ResultsScene({ active }: { active: boolean }) {
  return (
    <div className="absolute inset-0 flex flex-col items-stretch justify-center max-[1100px]:justify-start p-5 sm:p-8 bg-yellow overflow-y-auto">
      <span className="absolute top-3 left-4 sm:top-5 sm:left-6 font-mono text-[10px] sm:text-[11px] tracking-widest uppercase text-[#666]">
        <span className="text-ink font-bold">SCENE</span> Â· RESULTS
      </span>
      <div className="px-2 mt-6">
        <div className="font-display text-[18px]">Speedup over CPU</div>
        <div className="font-mono text-xs text-[#555] mb-4">gmean 5,432Ã— across 7 benchmarks</div>
          ...
          {BARS.map((b, i) => (
            ...
            >
              <div className="font-mono text-[12px]">
                {b.label}
                {b.winner && <span className="text-coral"> â˜…</span>}
              </div>
